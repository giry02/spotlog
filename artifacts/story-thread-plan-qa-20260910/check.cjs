  ...
  try {
    for (const [kind, count] of [['development-summary',8],['development-plan',33],['ai-guide-plan',57],['ai-guide-summary',5]]) {
      ...
      for(let i=1;i<=count;i++) {
        ...
        if(kind==='development-summary'||(kind==='development-plan'&&[11,14,15,16,18,23,24,27].includes(i))||(kind==='ai-guide-plan'&&[45,46].includes(i))) await page.screenshot({path:__dirname+'/'+kind+'-'+i+'.png'});
      }
