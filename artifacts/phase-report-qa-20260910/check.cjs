...
(async () => {
  ...
  try {
    for (const [kind, count] of [['development-summary',7],['development-plan',32],['ai-guide-plan',57],['ai-guide-summary',5]]) {
      ...
      if(kind==='development-plan') for(let i=1;i<=18;i++) assert(content.includes('F'+String(i).padStart(2,'0')));
      for(let i=1;i<=count;i++) {
        ...
        if(kind==='development-summary'||(kind==='development-plan'&&[11,12,13,14,15,16,17,21,31].includes(i))||(kind==='ai-guide-plan'&&[45,46].includes(i))) await page.screenshot({path:__dirname+'/'+kind+'-'+i+'.png'});
      }
