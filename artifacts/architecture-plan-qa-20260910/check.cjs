...
(async () => {
  ...
  try {
    for (const [kind, count] of [['development-summary',11],['development-plan',36],['ai-guide-plan',57],['ai-guide-summary',5]]) {
      ...
      for(let i=1;i<=count;i++) {
        ...
        if(kind==='development-summary'||(kind==='development-plan'&&[11,12,13,14,17,18,27].includes(i))||(kind==='ai-guide-plan'&&[45,46].includes(i))) await page.screenshot({path:__dirname+'/'+kind+'-'+i+'.png'});
      }
