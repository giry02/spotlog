...
import { buildSavedTravelDraft, interpretTravelPrompt, localTravelDraftProvider, travelRegionOptions, type TravelConditions, type TravelDraftProvider, type TravelDraftResult, type TravelPace, type TravelTransport } from './aiTravelDraft';
...
export function AiTravelSheet({ initialPrompt = '', savedDayCount, provider = localTravelDraftProvider, places, onClose, onCreate }: AiTravelSheetProps) {
  ...
  const changePrompt = (next: string) => { invalidateGeneration(); setPrompt(next); setOverrides({}); };
  const changeConditions = (next: typeof overrides) => { invalidateGeneration(); setOverrides(next); };
  const generate = async () => {
    if (generating.current || saving.current) return;
    generating.current = true;
    const request = ++requestId.current;
    setError(''); setBusy(true);
    try {
      const result = await provider.generate({ prompt, overrides }, places);
    ...
    } catch { if (request === requestId.current) setError('초안을 만들지 못했어요. 입력한 내용은 유지되니 다시 시도해 주세요.'); }
    finally { if (request === requestId.current) { generating.current = false; setBusy(false); } }
  };
  const save = () => {
  ...
  return <BottomSheet title={preview ? '여행 초안 미리보기' : 'AI 여행 만들기'} description="로컬 추천 미리보기 · 외부 AI 미연결" onClose={close}>
    <div className="ai-travel-sheet">
      {!preview ? <form className="ai-travel-form" onSubmit={(event) => { event.preventDefault(); void generate(); }}>
        <Field label="어떤 여행을 하고 싶으세요?" hint="지역·기간·취향을 적으면 등록된 장소에서 골라요. 실제 AI 답변이 아닌 조건 기반 미리보기예요.">
          <textarea rows={4} maxLength={1000} value={prompt} placeholder="부산에서 1박 2일, 바다를 보고 싶어요. 카페는 빼고 많이 걷지 않았으면 좋겠어요." onChange={(event) => changePrompt(event.target.value)} />
        </Field>
        {!prompt && <button type="button" className="phase-search-reset ai-travel-example" onClick={() => changePrompt('부산 1박 2일, 바다를 보고 카페는 빼고 덜 걷고 싶어요.')}>예시 문장 넣기 <ChevronRight size={14} /></button>}
        <div className="ai-travel-conditions">
          <Field label="여행 지역"><select value={conditions.region} onChange={(event) => { changeConditions({ ...overrides, region: event.target.value }); setError(''); }}><option value="">지역 선택</option>{regionOptions.map((region) => <option value={region} key={region}>{region}</option>)}</select></Field>
          <Field label="여행 기간"><select value={conditions.dayCount >= 1 && conditions.dayCount <= 7 ? conditions.dayCount : ''} onChange={(event) => { changeConditions({ ...overrides, dayCount: Number(event.target.value) }); setError(''); }}><option value="" disabled>기간 선택</option>{Array.from({ length: 7 }, (_, index) => index + 1).map((days) => <option value={days} key={days}>{periodLabel(days)}</option>)}</select></Field>
          <Field label="여행 속도"><select value={conditions.pace} onChange={(event) => changeConditions({ ...overrides, pace: event.target.value as TravelPace })}>{Object.entries(paceLabels).map(([key, label]) => <option value={key} key={key}>{label}</option>)}</select></Field>
          <Field label="이동수단 · 경로 검증 전"><select value={conditions.transport} onChange={(event) => changeConditions({ ...overrides, transport: event.target.value as TravelTransport })}>{Object.entries(transportLabels).map(([key, label]) => <option value={key} key={key}>{label}</option>)}</select></Field>
        </div>
        ...
        <button className="phase-search-reset ai-travel-back" onClick={() => { if (savedDayCount !== undefined) close(); else { setPreview(null); setError(''); } }} disabled={busy}><ArrowLeft size={16} />{savedDayCount !== undefined ? '저장 목록으로' : '조건 바꾸기'}</button>
