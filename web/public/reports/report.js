...
(() => {
  ...
  function setupCost(root){
    ...
    const update=()=>{
      ...
      form.querySelector('[data-cost-status]').textContent=ready?'입력 항목의 월 소계임. 초기·변경 색인과 인건비는 별도 예산이며, 세금·환율·최소 계약금·초과료도 확인 필요함.':'빈 단가는 무료가 아님. 같은 원화 기준의 검증된 단가를 모두 입력해야 합계를 계산함.';
